...
def get_base_pQ(pb_client, body_id):
    """
    Returns position, orientation of the base link.
    """
    p, Q = pb_client.getBasePositionAndOrientation(body_id)
    p, Q = np.array(p), np.array(Q)
    if not xyzw_in:
...
def get_link_pQvw(pb_client, body_id, indices=None):
    ...
    ls = pb_client.getLinkStates(body_id, indices, computeLinkVelocity=True, computeForwardKinematics=True)
...
def get_link_pQ(pb_client, body_id, indices=None):
    """
    Returns positions, orientations given link indices.
    Please use get_base_pQ for the base link.
    """
    if indices is None:
        indices = range(pb_client.getNumJoints(body_id))
    ...
    ls = pb_client.getLinkStates(body_id, indices, computeForwardKinematics=True)
...
def get_link_pQvw_joint_frame(pb_client, body_id, indices=None):
    """
    Returns positions, orientations, linear and angular velocities given link indices.
    Please use get_base_pQvw for the base link.
    """
    ...
    ps = [np.array(ls[j][4]) for j in range(num_indices)]
    if not xyzw_in:
        Qs = [
            quaternion.Q_op(np.array(ls[j][5]), op=['change_order'], xyzw_in=True) \
            for j in range(num_indices)]
    else:
        Qs = [np.array(ls[j][5]) for j in range(num_indices)]
    vs = [np.array(ls[j][6]) for j in range(num_indices)]
    ws = [np.array(ls[j][7]) for j in range(num_indices)]
...
def get_link_pQ_joint_frame(pb_client, body_id, indices=None):
    """
    Returns joint positions, orientations given link indices. (i.e. without link CoM offset)
    Please use get_base_pQvw for the base link.
    """
    if indices is None:
        indices = range(pb_client.getNumJoints(body_id))
...
def compute_com_and_com_vel(pb_client, body_id, indices=None):
    ...
    for i in indices:
        ...
        if i == -1:
            ...
            ls = pb_client.getLinkState(body_id, i, computeLinkVelocity=True, computeForwardKinematics=True)
